...
max:buttons(my_table.join(awful.button ({}, 1, function() local c = client.focus
...
-- MEM
local mem = lain.widget.mem({settings = function()
...
-- CPU
local cpu = lain.widget.cpu({settings = function()
...
-- Battery
local bat = lain.widget.bat({settings = function()
...
-- ALSA volume
theme.volume = lain.widget.alsa({settings = function()
...
-- Net
local net = lain.widget.net({settings = function()
...
s.mywibox = awful.wibar({
        ...
        height = dpi(34),
        type = 'dock',
        shape = function(cr, w, h) gears.shape.rounded_rect(cr, w, h, dpi(0))end,
    })
